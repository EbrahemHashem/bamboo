---
name: bm-anomaly-detector
cron: "0 */4 * * *"
timezone: Europe/Amsterdam
mcps: [airtable]
owner: bamboo
status: active
origin: bamboo-anomaly-detector.json
---

# Anomaly Detector (Cross-Engine)

Every 4 hours — scans Daily KPIs, Ad Variants, Email Performance tables for anomalies across all 6 Bamboo engines. Logs to Anomaly_Log + Telegram alert if any detected.
...
## 1. Fetch baseline (7d Daily KPIs) + today
...
## 2. Compute baselines + compare today
...
- Airtable read fail → stop with error log + Telegram
- Single anomaly log fail → continue, batch remaining
- Zero anomalies → silent success (no Telegram)
...
/bm-anomaly-detector
```
Expect: rows in `Anomaly_Log` if detections, Telegram alert if any.
