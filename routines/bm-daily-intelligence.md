---
name: bm-daily-intelligence
cron: "30 7 * * *"
timezone: Europe/Amsterdam
mcps: [airtable]
owner: bamboo
status: active
origin: bamboo-daily-intelligence.json
---

# Daily Intelligence Brief

Daily 07:30 — aggregate overnight KPIs across all 6 Bamboo engines (ad spend + revenue, conversions, customers, email, B2B pipeline), upsert Daily_KPIs table, send morning Telegram brief to Nathan with one insight + one action.
...
## 2. Ad spend + revenue (from Ad_Variants table — stored by daily-ad-data-pull)
...
## 4. Email performance + B2B pipeline
...
## 7. Build morning Telegram brief
...
- Any single fetch fail → continue with partial data, note in summary
- All fetches fail → stop + Telegram alert
- Zero ad spend (no campaigns) → valid, reports 0 with note

## Verification

```bash
/bm-daily-intelligence
```
Expect: 1 row in `Daily_KPIs`, Telegram brief to Nathan.
