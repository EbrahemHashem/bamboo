---
name: bm-retention-flow-monitor
cron: "0 8 * * *"
timezone: Europe/Amsterdam
mcps: [airtable]
owner: bamboo
status: active
origin: bamboo-retention-flow-monitor.json
---

# Retention Flow Monitor

Daily 08:00 — check Klaviyo flow health (open rate, click rate, revenue attribution), compare last 7d vs prior 7d, flag critical degradation.
...
## 1. Fetch all flows + last 7d metrics
...
## 2. Per flow: pull 7d + prior 7d metrics
...
- Klaviyo API fail → stop, Telegram alert
- Single flow fail → log warning, continue
- Zero live flows → status=empty

## Verification

```bash
/bm-retention-flow-monitor
```
Expect: rows in `Email_Performance`, Telegram only if critical.
